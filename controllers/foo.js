...
exports.get = function(req, res, next) {
  ...
  db.collection('foo').find().toArray((err, data) => {
...
exports.getById = function(req, res, next) {
  const db = req.app.locals.db;
  db.collection('foo').findOne({ '_id': new ObjectID(req.params.id) }, (err, data) => {
    if (err) return next(createError(400, 'Error querying database'));
...
exports.insert = function(req, res, next) {
  const db = req.app.locals.db;
  db.collection('foo').insertOne(req.body, (err, data) => {
    if (err) return next(createError(400, 'Error querying database'));
...
exports.updateById = function(req, res, next) {
  const db = req.app.locals.db;
  db.collection('foo').updateOne({ '_id': new ObjectID(req.params.id) },
  { $set: req.body },
  (err, data) => {
    if (err) return next(createError(400, 'Error querying database'));
...
exports.deleteById = function(req, res, next) {
  const db = req.app.locals.db;
  db.collection('foo').deleteOne({ '_id': new ObjectID(req.params.id) },
  (err, data) => {
    if (err) return next(createError(400, 'Error querying database'));
